...
export const useChatStore = create((set,get) => ({
  ...
  getMessages: async (userId) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.msg || "Failed to load messages");
    } finally {
  ...
  sendMessage: async (messageData) => {
    console.log("🛠️ sendMessage in store called with:", messageData);
    ...
    try {
      ...
      set({ messages: [...safeMessages, data] });
      console.log(messages);
    } catch (err) {
      console.error("Error in sendMessage:", err);
      toast.error(err?.response?.data?.msg || err.message || "Failed to send message");
    }
  },
